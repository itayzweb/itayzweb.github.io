...
function checkstat(){ //התחלת פונקציה 
    ...
    if(gal){//trueהקוד בתוך הצהרה איתקים עם המישתנה היה שווה ל1 או
...
function logout() {
        sessionStorage.setItem("logdin", JSON.stringify(false));
        EndRide()//EndRide-קוראים לפונקציה 
...
function lock(lock){
    ...
    gal=true;//משנה את הערך של המשתנה
    cride();
...
function cride() {
    ...
    var cridePath = 'id:' + uid + '/cride';
    
    firebase.database().ref(cridePath).on('value', function(snapshot) {
        var crideData = snapshot.val(); // Retrieve data from Firebase
...
function history() {
    ...
    dismax=JSON.parse(sessionStorage.getItem("distancemax"));
    ...
    firebase.database().ref(`id:${uid}/ride:${cride}/avgdis`).set(dismax/time);
...
function average(){
    ...
    kmhmax=JSON.parse(sessionStorage.getItem("kmhmax")); 
    dis=JSON.parse(sessionStorage.getItem("distance"));
    dismax=JSON.parse(sessionStorage.getItem("distancemax"));  
    bpm=JSON.parse(sessionStorage.getItem("bpm"));
    bpmmax=JSON.parse(sessionStorage.getItem("bpmmax"));

    sessionStorage.setItem("kmhmax", JSON.stringify(kmhmax+kmh));
    sessionStorage.setItem("distancemax", JSON.stringify(dismax+dis));
