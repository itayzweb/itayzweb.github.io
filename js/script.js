...
function prox(dat){//התחלת פונקציה עם ערך
    ...
    else if(dat.val()>=7)//הקוד בתוך הצהרה יתקיים עם ההצהרה היה נכון וההצהרה הקודמת לא תהיה הכון
...
function alerts(){
    ...
    if(alerttimer==1){
        if(alerttimer==0)
            ...
            alerttimer+1
            if(time-alerttimer<2)
                firebase.database().ref('/data' ).set(2);
            else{
                firebase.database().ref('/data' ).set(0);
                alerttimer=0;
                alertchecker=0;
            }
        }
...
function average(){
    ...
    if(rpsmax>0){
        kmhs=parseFloat(2*3.14159265359*radius*(rps*60)/1000)
        kmhs=kmhs.toFixed(0)
        kmh.innerHTML=kmhs
    }
    else
    ...
    sessionStorage.setItem("rpsmax", JSON.stringify(parseFloat(2*3.14159265359*radius*(rps*60)/1000)+rpsmax));
