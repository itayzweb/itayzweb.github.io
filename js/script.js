...
function initMap() {
    const latitude = JSON.parse(sessionStorage.getItem("latitude"));
    const longitude = JSON.parse(sessionStorage.getItem("longitude"));
    ...
    if (!map) {
        ...
        // Add event listener for idle event
        google.maps.event.addListenerOnce(map, 'idle', function() {
            // Map rendering is complete, capture the map image
            captureMap();
        });
    }
...
function captureMap() {
    ...
    // Calculate the width and height of the map based on the bounds
    var mapWidth = mapp.offsetWidth;
    var mapHeight = mapp.offsetHeight;
    
    // Create a new map canvas element with the same dimensions
    var mapCanvas = document.createElement('canvas');
    mapCanvas.width = mapWidth;
    mapCanvas.height = mapHeight;

    // Set the map viewport to the bounds
    ...
    var options = {
        ...
        width: mapWidth,
        height: mapHeight,
        useCORS: true // Allow cross-origin requests for map tiles
    };

    // Capture the map as an image
    html2canvas(mapp, options).then(function(canvas) {
        // Convert the canvas to a Blob object
        canvas.toBlob(function(blob) {
            // Reset the map viewport to its original position
            map.fitBounds(bounds);

            // Upload the Blob to Firebase Storage
            var imageName = "map"; // Set your desired image name here
            var storageRef = firebase.storage().ref();
            var cride = JSON.parse(sessionStorage.getItem("cride"));
            var uid = JSON.parse(localStorage.getItem("uid"));

            // Upload the Blob to Firebase Storage
            storageRef.child(uid + '/' + cride + '/' + imageName).put(blob).then(function() {
...
document.addEventListener("DOMContentLoaded", function() {//מקשיב לשינוי פיירבייס בשביל התחלת פונקציה
    ...
    firebase.database().ref('/kmh').on('value', kmh);//לוקח נתונים מהפיירבייס ושולח אותם לפוקציה
